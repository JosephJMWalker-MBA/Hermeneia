...
def create_app(
    ...
    *,
    ...
    def _empty_literal_concordance_response(q: str) -> dict:
        return {
            "query": q,
            "count": 0,
            "passage_count": 0,
    ...
    def _literal_concordance_for_rows(
        ...
        *,
        ...
        for r in rows:
            text = r["searchable_text"] or ""
            occurrences = literal_occurrence_spans(text, query)
            if not occurrences:
                continue
            ...
            match[result_text_field] = text
            match.update(extra_fields.get(r["id"], {}))
    ...
    def api_search():
        ...
        active_ids = active_observation_ids(conn)
        id_to_index = {obs_id: index + 1 for index, obs_id in enumerate(active_ids)}
        concordance = _literal_concordance_for_rows(
            all_rows,
            query=q,
        ...
        concordance.pop("all_results", None)
        return jsonify({
            "query": q,
            **{
                key: value
                for key, value in concordance.items()
                if key != "limited_results"
            },
            "results": concordance["limited_results"],
        })
    ...
    def api_e10_observations():
        ...
        rows = conn.execute(
            ...
            SELECT o.id, o.page, o.paragraph, o.sentence, o.raw_text,
                   COALESCE(od.normalized_text, o.raw_text) AS searchable_text,
                   o.source_locator, o.source_document_id, o.source_extraction_id,
                   sd.original_filename, sd.source_role
            FROM observations o
            LEFT JOIN observation_derived od ON od.observation_id = o.id
            LEFT JOIN source_documents sd ON sd.id = o.source_document_id
        ...
        if q:
            ...
            observations = concordance.pop("limited_results")
            concordance.pop("all_results", None)
            conn.close()
            return jsonify({
                "query": q,
