...
Comparative experiments in literary analysis suggest that divergence between investigators may begin before interpretation, at the point where each reader decides what deserves attention. If confirmed, this would make the record of human attention a foundational epistemic artifact rather than an annotation.

This responsibility is stated here as a hypothesis because the architecture predicts it and the initial experiments suggest it — but Hermeneia's own constitutional standard requires that ontological commitments earn their existence through repeated practice rather than theoretical appeal.
...
Three distinct compressions, non-contradictory:
...
These are not three different claims about three different novels. They are different summaries of the same evidence under different governing questions. Together they compose a picture no single execution produced.

In this sense, the Gatsby executions did not expand the novel into arbitrary
commentary. They traversed different regions of the same relational structure.
The archive remained constant. The extraction path changed.

This is why the compression claim should be read as a methodological finding
rather than a theory of literary essence. A novel can be understood as a
densely encoded relational structure, but Hermeneia does not claim to recover
the one true decompression. It asks whether a given recovery path followed
evidence, preserved provenance, and made its governing question inspectable.
...
The processual-adaptive execution did something the other two did not. Its research design section explicitly described its own investigative methodology:

> **观察 → 分类 → 问题生成 → 收集证据 → 修正假设 → 再次验证**  
> *(Observation → Classification → Question Generation → Evidence Collection → Hypothesis Revision → Re-verification)*

And the governing epistemological principle:

> **文本先于解释，证据先于理论，修正先于结论。**  
> *Text precedes interpretation, evidence precedes theory, revision precedes conclusion.*

This is not another literary conclusion. It is a replication of the investigative process itself. The investigator, beginning from a different governing question and cultural tradition, independently reconstructed the same cognitive sequence that the Hermeneia architecture implements.

The investigator also recorded that an initial governing hypothesis — "harmony" — was revised when it failed to account simultaneously for the differences between Gatsby, Nick, and Wilson. The revision produced the actual governing concept: continuous adaptation. This is the Architect function described in Section 5: not planning an output, but reconstructing the understanding that the evidence actually supports, including revising the hypothesis when evidence resists it.

This observation is qualitatively different from the literary findings. The other executions tested whether different investigators reach different conclusions from the same corpus. This execution showed an investigator independently describing — in the body of the investigation itself — the same workflow the architecture was built to support. That is an unexpected form of convergent evidence for the architecture's claim about how disciplined inquiry is organized.

It is recorded here as a single observation. It has not been replicated. It warrants further investigation.
